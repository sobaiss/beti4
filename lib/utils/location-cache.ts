 ...
 */
function encodeForStorage(data: any): string {
  try {
 ...
 */
function decodeFromStorage(encodedData: string): any {
  try {
...
function getLocationsFromCache(): Location[] | null {
  try {
    ...
    if (!Array.isArray(decodedLocations)) {
      console.warn('Cached locations data is not an array, clearing cache');
      clearLocationsCache();
      return null;
    }
